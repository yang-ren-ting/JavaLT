...
//上一页
		function dopre(page){
			...
			window.location.href="topic.do?boardId=${requestScope.board.boardId}&pageNo="+(page-1);
		}
		//下一页
...
<table class="t1" cellspacing="0">
  ...
  <tbody>
    <p:forEach items="${requestScope.topics}" var="one">
      <tr>
       <td>
      <p:if test="${one.istop==1}">
      				<span class="ding">顶</span>
      			</p:if>
      <p:if test="${one.isgood==1}">
      				<span class="jing">精</span>
      </p:if>
      <p:if test="${one.istop==0&&one.isgood==0}">
      	 <img src="${pageContext.request.contextPath }/imgs/topic.gif">
      </p:if>
