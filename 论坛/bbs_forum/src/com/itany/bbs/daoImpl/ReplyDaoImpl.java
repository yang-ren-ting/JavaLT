...
//	}
	  ...
	  public int getReplyCount(int topicId)
	    {
	        String sql="select count(*) from replys where state>0 and topicid="+topicId;
	        ResultSetHandler<Long> rsh= new ScalarHandler<Long>();
	        try
	        {
	            Long ret=qr.query(conn, sql, rsh);
	...
	public List<Reply> findReplyByPage(int topicId, Paginate page) {
		...
		ResultSetHandler<List<Reply>> rsh = new BeanListHandler<Reply>(Reply.class);
...
//		this.templete.update(sql, params);
	...
	public int getReplyCountAll() {
		  String sql="select count(*) from replys where state>0 ";
	       ResultSetHandler<Long> rsh= new ScalarHandler<Long>();
	       try
	        {
	            Long ret=qr.query(conn, sql, rsh);
	...
	@Override
	public List<Reply> findallReply(Paginate page) {
		String sql = "SELECT * FROM replys WHERE state = 1 ORDER BY postdate DESC LIMIT ?,?";
        Integer start = (page.getPageNo() - 1) * page.getPageSize();
        Object[] params = new Object[2];
        params[0] = start;
	...
	@Override
	public int getReplyCountbybd(String boardname) {
		 String sql="select count(*) from replys where state = 1 and topicid in (select topicId from topics where boardid=(select boardId from boards where boardName=?))";
	...
	@Override
	public List<Reply> findReplysBybdname(String boardname, Paginate page) {
		String sql = "SELECT * FROM replys WHERE state = 1 and topicid in (select topicId from topics where boardid=(select boardId from boards where boardName=?))ORDER BY postdate DESC LIMIT ?,?";
		 Integer start = (page.getPageNo() - 1) * page.getPageSize();
	        Object[] params = new Object[3];
	        params[0] = boardname;
	        params[1] = start;
